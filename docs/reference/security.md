...
> You will need to o enable experimental features in both the host and guest's tool settings to use it. In VS Code, set **"liveshare.features":"experimental"** to **settings.json**. In Visual Studio, set **Tools > Options > Features** to **Experimental**. Be aware that these features are still in flight and subject to change.
