...
| VS Code | <strong>Join:</strong> You were <strong>not prompted / able to launch VS Code</strong> after opening the join page in a browser. |  Tips: <ul><li>Be sure you've <i>started the VS Code at least once and waited for the install to complete in the status bar.</i></li><li>If that doesn't work, try running the "Live Share: Launcher Setup" command.</li><li>Finally, see [joining manually](manual-join.md) as a workaround.</lu></ul> If you hit this problem, we would love to see what might be happening, so please [log a bug](https://aka.ms/vsls-new-issue). |
| VS | <strong>Join:</strong> You were <strong>not prompted / able to launch VS </strong> after opening the join page in a browser. |  See [join manually](manual-join.md).<br /><br /> We would also love to see your logs, so please please [log a bug](https://aka.ms/vsls-problem) using VIsual Studio's "Report a Problem..." feature. |
| All | <strong>Join:</strong> You would prefer to <strong>paste the join link directly into VS / VS Code</strong> rather than clicking on the web link. | See [join manually](manual-join.md). |
## Connectivity
