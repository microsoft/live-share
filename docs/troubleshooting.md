...
|------|---------|------------|
| VS | The extension installer <strong>cannot find a version of Visual Studio</strong> to use when trying to install the Visual Studio Live Share extension. | Visual Studio Live Share requires **VS 2017 version 15.6** or higher for both hosts and guests. Install the latest stable [update of VS 2017](https://visualstudio.com/vs/) and retry.|
...
|------|------|----------------|
| All | You are using a <strong>proxy</strong> and are seeing a number of connectivity problems | Proxy settings can be tricky particularly in the case of VS Code. As a first step, set an **HTTP_PROXY** and **HTTPS_PROXY** environment variable **globally** and retry in **direct mode**. See the [connectivity requirements](reference/connectivity.md#proxies) article for details.<br /><br />If this does not resolve the problem, we are investigating and working on fixes for number of issues when connecting via a proxy. Please let us know what you are seeing in [this issue](https://github.com/MicrosoftDocs/live-share/issues/86) and be sure to mention if you are in VS or VS Code along with the OS you are running. |
